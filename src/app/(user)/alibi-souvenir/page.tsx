...
// Add this with your existing imports if not already present

// Define possible API response type
type UserSouvenirRequestResponseAPI =
  | UserSouvenirRequestResponse[]
  | { results: UserSouvenirRequestResponse[] };
...
const AlibiSouvenir = () => {
  ...
  const fetchSouvenirRequests = async () => {
    ...
    try {
      const data =
        (await userApiClient.getUserSouvenirRequests()) as UserSouvenirRequestResponseAPI;

      if (Array.isArray(data)) {
        setSouvenirRequests(data);
      } else if (data && Array.isArray(data.results)) {
        setSouvenirRequests(data.results);
      } else {
        setSouvenirRequests([]);
      }
    } catch (err: any) {
