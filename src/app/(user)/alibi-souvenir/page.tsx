...
import { GalleryItem, UserSouvenirRequestResponse } from "@/infrastructure/user/utils/types";
...
const AlibiSouvenir = () => {
  ...
  const [souvenirRequests, setSouvenirRequests] = useState<UserSouvenirRequestResponse[]>([]);
  const [loadingRequests, setLoadingRequests] = useState(false);
  const [requestError, setRequestError] = useState<string | null>(null);
  const [requestPage, setRequestPage] = useState(1);
  const REQUESTS_PER_PAGE = 6;
  const totalPages = Math.ceil(souvenirRequests.length / REQUESTS_PER_PAGE);
  const paginatedRequests = souvenirRequests.slice((requestPage - 1) * REQUESTS_PER_PAGE, requestPage * REQUESTS_PER_PAGE);
  ...
  const fetchSouvenirRequests = async () => {
    ...
    try {
      ...
      setSouvenirRequests(Array.isArray(data) ? data : (Array.isArray(data.results) ? data.results : []));
    } catch (err: any) {
  ...
  const onOrderSubmit = async (data: any) => {
    ...
    try {
      ...
      const res = await fetch("https://15.206.185.80/gallery/souvenir-requests", {
        method: "POST",
        headers: {
      ...
      toast.error(err.message || "注文に失敗しました。もう一度お試しください。");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="main_gradient_bg min-h-screen">
      <Menu text="アリバイギャラリーからお土産注文" position="left" className="pl-10" />
      <div className="max-w-6xl mx-auto px-4">
        ...
        {activeTab === 'form' && (
          <>
          <h2 className="text-2xl font-bold text-white mt-10 mb-8 text-center">ギャラリーからお土産を選択</h2>
          {loading ? (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-xl">
            <div className="glass-card bg-gradient-to-br from-white/20 to-blue-100/10 backdrop-blur-2xl rounded-3xl shadow-2xl p-0 max-w-lg w-full relative flex flex-col items-center border border-white/30 overflow-hidden">
              ...
              <div className="w-full px-8 py-6 flex flex-col items-center">
                <h3 className="text-xl font-bold mb-2 text-gray-900 drop-shadow text-center">{orderModal.title}</h3>
                <form className="w-full flex flex-col gap-4" onSubmit={handleSubmit(onOrderSubmit)}>
                  <div>
                    ...
                    />
                    {errors.quantity && <span className="text-xs text-red-500">数量は1以上で入力してください</span>}
                  </div>
                  <div>
                    ...
                    />
                    {errors.description && <span className="text-xs text-red-500">説明は必須です</span>}
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="glass-input w-full p-3 rounded-lg border border-white/20 bg-white/30 text-gray-900"
                    />
                    {errors.desire_delivery_date && <span className="text-xs text-red-500">納品希望日は必須です</span>}
                  </div>
                  <Button
                    type="submit"
                    variant="glassSec"
              ...
              <>
                <div className="flex flex-col gap-0 divide-y divide-white/10">
                  {paginatedRequests.map((req, idx) => (
                    <div
                      key={req.uid}
                      className={`flex items-center gap-6 py-4 px-2 glass-card bg-white/10 hover:bg-white/20 transition-all rounded-2xl ${idx === 0 ? 'mt-0' : ''}`}
                    >
                      {/* Images */}
                      <div className="flex gap-2 min-w-[80px]">
                        {Array.isArray(req.media_files) && req.media_files.length > 0 ? (
                          req.media_files.slice(0, 3).map((file, i) => (
                            ...
                            >
                              <img
                                src={file.file.startsWith('http') ? file.file : `https://15.206.185.80${file.file}`}
                          ...
                          <div className="w-16 h-16 flex items-center justify-center bg-white/10 rounded-xl text-gray-400 text-xl">?</div>
                        )}
                      </div>
                      {/* Details */}
                      <div className="flex-1 flex flex-col gap-1 min-w-0">
                        <div className="flex items-center justify-between">
                          ...
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold shadow-sm border font-jakarta tracking-wide whitespace-nowrap ${req.request_status === 'approved' ? 'bg-green-100 text-green-700 border-green-200' : req.request_status === 'pending' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : 'bg-blue-100 text-blue-700 border-blue-200'}`}>{req.request_status}</span>
                        </div>
                        <div className="text-gray-200 text-xs mb-1 line-clamp-2 font-medium font-jakarta truncate">{req.special_note}</div>
                        <div className="flex items-center gap-4 text-xs text-gray-300 font-jakarta mt-1">
                          <span className="font-medium">数量: <span className="font-normal">{req.quantity}</span></span>
                          <span className="font-medium">納期: <span className="font-normal">{req.desire_delivery_date ? new Date(req.desire_delivery_date).toLocaleDateString() : 'N/A'}</span></span>
                        </div>
                      </div>
                    </div>
                  ))}
